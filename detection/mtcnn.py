...
from .utils_mtcnn import detect_face, extract_face
...
class PNet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            state_dict_path = os.path.join(str(pathlib.Path(__file__).parent.absolute()), 'data/pnet.pt')
...
class RNet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            state_dict_path = os.path.join(str(pathlib.Path(__file__).parent.absolute()), 'data/rnet.pt')
            state_dict = torch.load(state_dict_path)
            self.load_state_dict(state_dict)
...
class ONet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            state_dict_path = os.path.join(str(pathlib.Path(__file__).parent.absolute()), 'data/onet.pt')
            state_dict = torch.load(state_dict_path)
            self.load_state_dict(state_dict)
...
class MTCNN(nn.Module):
    ...
    def forward(self, img, save_path=None, return_prob=False):
        ...
        if not self.keep_all:
            batch_boxes, batch_probs, batch_points = self.select_boxes(batch_boxes, batch_probs, batch_points,
    ...
    def select_boxes(self, all_boxes, all_probs, all_points, imgs, method='probability',
